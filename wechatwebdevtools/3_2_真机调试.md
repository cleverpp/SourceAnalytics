...
5. remoteDebugWindow窗口弹出并加载url： 启动了一个调试node的chrome devTools
    ...
    - RemoteDebug 监听initSuccess，触发真机调试窗口打开。
6. 
...
3. 初始化调试实例RemoteDebug js/6c6f4dde020ed24929c4c4367c42d34d.js
    ...
    ```
    - 监听initsuccess：打开了remoteDebug窗口，窗口地址为：`chrome-devtools://devtools/bundled/inspector.html?experiments=true&ws=${inspectUrl.replace(/^ws\:\/\//i, "")}`
...
4. 子进程remote-helper.js, websocket clinet
    ...
    - initWs，初始化Websocket客户端，连接上websocket Server后，notifyMaster
     ...
     - 根据Chrome DevTools的原理，那么node inspector会作为Chrome DevTools工具的ws的服务端---》回到入口文件中的WebSocketServer。
