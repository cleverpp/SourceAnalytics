...
2. 组件j，远程调试窗口的渲染 js/c9088b8c1e58e44a737f97a08c5a116e.js
    ...
    - 创建左侧调试窗口webview，设置UserAgent和url
        ```
        j.showDevTools(!0, c); // c=webview,作为devTools的容器
...
3. (chrome <webview> Tag)[https://developer.chrome.com/apps/tags/webview]

## 通信流程
存在的通信对象：
1. WebSocketServer（端口为：global.messageCenterPort)。
2. 远程服务端RemoteUrl(wss://wxagame.weixin.qq.com/remote/) 和 Messager（ws client)
3. DebugServer(ws server) 和 remote-helper.js(ws client)
4. RemoteDebug实例、DebugServer实例、Messager实例 都是EventEmmiter对象。

通信流程-vmReady：
1. remote-helper.js 启用node inspector并获取inspectorUrl后 发送vmReady消息到DebugServer。
2. DebugServer接受到消息后拼接完整的url后发送initsuccess消息
3. RemoteDebug实例接受到initsuccess消息后输出：console.log('subscribed remote window shown'）

通信流程-statuschange，真机调试窗口显示
1. 用户扫码成功后，远程服务端RemoteUrl发送消息给 Messager ： MessageNotify类型
2. Messager接受到消息后，对消息进行解码，并根据消息类型设置状态：setStatus(q.ActiveIdle)，发送statuschange事件
3. RemoteDebug实例接收到statuschange消息后，调用onStatusUpate，在该函数中调用this.props.windowActions.setRemoteDebugWindow触发真机调试窗口显示。

远程调试-自定义面板AppData
1. idepluginManager = require('./f5a748840b272d2bf0223c95f6c8dbe3.js')
